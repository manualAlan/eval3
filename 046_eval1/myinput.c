...
void parse_planet_info(planet_t * planet, char * line) {
  ...
  if (strlen(temp) >= MAX_PLANET_NAME_LEN) {
    free(line_copy);
    fprintf(stderr, "Planet name longer than maximum (31) length\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (*endptr != '\0') {
    free(line_copy);
    fprintf(stderr, "Invalid format for orbital radius.\n");
    exit(EXIT_FAILURE);
  }
  if (planet->orbital_radius <= 0.0) {
    free(line_copy);
    fprintf(stderr, "orbital_radius can't be non-positive\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (*endptr != '\0') {
    free(line_copy);
    fprintf(stderr, "Invalid format for period.\n");
    exit(EXIT_FAILURE);
  }
  if (planet->year_len <= 0.0) {
    free(line_copy);
    fprintf(stderr, "year_len can't be non-positive\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (*endptr != '\0') {
    free(line_copy);
    fprintf(stderr, "Invalid format for initial position.\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (planet->init_pos < 0.0) {
    free(line_copy);
    fprintf(stderr, "init_pos can't be non-positive\n");
    exit(EXIT_FAILURE);
  }
