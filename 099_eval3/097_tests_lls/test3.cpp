...
class Tester {
 public:
  ...
  // Additional test for duplicates, find, and complex removal cases
  void testAdditionalCases() {
    IntList il;

    // Adding and removing duplicate elements
    il.addBack(5);
    il.addBack(5);
    il.addBack(10);
    il.addBack(5);
    assert(il.getSize() == 4);
    bool removed = il.remove(5);  // Should remove only the first occurrence of 5
    assert(removed == true);
    assert(il.getSize() == 3);
    assert(il[0] == 5 && il[1] == 10 && il[2] == 5);

    // Find method tests for existing and non-existing elements
    assert(il.find(10) == 1);   // Element 10 exists at index 1
    assert(il.find(5) == 0);    // Only the first occurrence of 5
    assert(il.find(99) == -1);  // Non-existing element should return -1

    // Adding to back after multiple removals to verify list usability
    il.addBack(20);
    assert(il.getSize() == 4);
    assert(il[3] == 20);

    // Edge case: remove all elements to make list empty
    il.remove(5);
    il.remove(10);
    il.remove(5);
    il.remove(20);
    ...
    // Test that adding to the emptied list works as expected
    il.addFront(30);
    assert(il.getSize() == 1);
    assert(il.head == il.tail && il.head->data == 30);
  }
};
